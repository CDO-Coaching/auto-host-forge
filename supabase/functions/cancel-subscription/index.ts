  ...
  try {
    ...
    if (customers.data.length === 0) {
      ...
      return new Response(JSON.stringify({ 
        success: true, 
        message: "No Stripe customer found, nothing to cancel" 
      }), {
    ...
    // Lister les abonnements actifs de ce customer
    ...
    // Annuler tous les abonnements actifs (cancel at period end pour ne pas facturer immédiatement)
    const cancelledSubs = [];
    for (const subscription of subscriptions.data) {
      logStep("Cancelling subscription", { subscriptionId: subscription.id });
      
      // Annuler à la fin de la période payée (l'athlète garde l'accès jusqu'à expires_at)
    ...
    return new Response(JSON.stringify({ 
      success: true, 
      message: `${cancelledSubs.length} subscription(s) will be cancelled at end of billing period`,
