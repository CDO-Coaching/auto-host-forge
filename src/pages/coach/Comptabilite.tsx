                    ...
                    {visibleEntries.map(entry => {
                      ...
                      <Card key={entry.id} id={`entry-${entry.id}`} className="shadow-sm transition-all border" style={{ borderLeft: `4px solid ${st.color}` }}>
                        <CardContent className="p-4 space-y-3">
                          ...
                          {/* Action rapide : ouvre la fenêtre "Marquer payé" (espèces/virement + montant) */}
                          {st.kind === "impaye" && (
                            <button
                              type="button"
              ...
              <div className="space-y-3">
                {debtorsList.map(debtor => (
                  <Card
                    ...
                    onClick={() => {
                      // Sélectionner un impayé → ouvrir directement la fenêtre "Marquer payé"
                      setShowDebtorsDialog(false);
                      openPayDialog(debtor);
                    }}
                  >
