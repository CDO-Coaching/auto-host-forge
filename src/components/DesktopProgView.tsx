                                            ...
                                            <TableRow key={`${exercise.id}-serie-${si}`} className="bg-muted/20">
                                              <TableCell className="pl-10 text-xs text-muted-foreground font-medium py-1">
                                                <span className="flex items-center flex-wrap gap-1">
                                                  Série {si + 1}
                                                  {(() => { const fb = getExerciseFeedback(selectedSession.id, exercise.exercice); const sd = fb?.serie_rpe_details?.[si]; if (!sd) return null; const isFailure = sd.modification_type === "failure"; const isTooEasy = sd.modification_type === "too_easy"; const plannedCharge = serie.charge || exercise.charge; const pc = (plannedCharge || "").trim(); const chargeIsUnknown = pc === "??" || /^(\d+(?:[.,]\d+)?)\s*-\s*(\d+(?:[.,]\d+)?)$/.test(pc); return (<span className="inline-flex flex-wrap gap-1">{sd.rpe != null && <span className="text-[10px] text-orange-500 font-medium">RPE {sd.rpe}</span>}{sd.actual_reps && <span className={`text-[10px] font-semibold ${isFailure ? "text-red-500" : isTooEasy ? "text-blue-500" : "text-orange-500"}`}>{isFailure ? "⬇" : isTooEasy ? "⬆" : "≠"} {sd.actual_reps} reps (prévu {serie.reps || exercise.reps})</span>}{sd.actual_charge && (chargeIsUnknown ? <span className="text-[10px] font-semibold text-orange-500">⚖️ {sd.actual_charge} kg</span> : <span className={`text-[10px] font-semibold ${isFailure ? "text-red-500" : isTooEasy ? "text-blue-500" : "text-orange-500"}`}>{isFailure ? "⬇" : isTooEasy ? "⬆" : "≠"} {sd.actual_charge} (prévu {plannedCharge})</span>)}</span>); })()}
                                                </span>
                                              </TableCell>
                                              <TableCell className="py-1">
