    ...
    <Card className="bg-card">
      ...
      <CardContent>
        {isEditing ? (
          ...
          <div className="space-y-4">
            {/* ── Section 1 : Repères (tuiles alignées) ── */}
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/70 mb-2">Repères</p>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                ...
                <div className="rounded-xl border border-border bg-muted/20 p-3 space-y-1">
                  ...
                  {vma ? (
                    <div className="flex items-baseline gap-1">
                      <span className="text-2xl font-bold text-primary">{vma.toFixed(1)}</span>
                  ...
                  {fcMax ? (
                    <>
                      ...
                      {fcMaxUpdatedAt ? (() => {
                        ...
                        return (
                          <p className={`text-[10px] font-medium leading-tight ${isOverdue ? "text-red-400" : isSoon ? "text-orange-400" : "text-muted-foreground/70"}`}>
                            {isOverdue ? `⚠️ Retest conseillé` : isSoon ? `⏳ Retest ${daysUntil}j` : `Retest dans ${daysUntil}j`}
                          </p>
                        );
                      })() : null}
                    </>
                ...
                <div className="rounded-xl border border-border bg-muted/20 p-3 space-y-1">
                  ...
                  {fcRepos ? (
                    <div className="flex items-baseline gap-1">
                      <span className="text-2xl font-bold text-blue-500">{fcRepos}</span>
                      <span className="text-xs text-muted-foreground">bpm</span>
                    </div>
                  ) : (
                    <p className="text-sm text-muted-foreground/60 italic">—</p>
            ...
            {/* ── Section 2 : Zones cardio (FCR / Karvonen) ── */}
            {fcMax && fcRepos ? (
              <div className="rounded-xl border border-border bg-muted/20 p-3">
                <div className="flex items-center justify-between mb-2.5">
                  <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/70">Zones cardio</p>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-[11px] text-muted-foreground">FC Réserve</span>
                    <span className="text-lg font-bold text-primary leading-none">{fcMax - fcRepos}</span>
                ...
                <div className="grid grid-cols-1 gap-1">
                  {[
                    ...
                    return (
                      <div key={z} className="flex items-center gap-2 py-0.5">
                        <div className={`w-2 h-2 rounded-full shrink-0 ${dot}`} />
                        <span className={`text-[11px] font-semibold w-5 ${color}`}>Z{z}</span>
                ...
                <p className="text-[10px] text-muted-foreground/60 mt-2">{fcMax} − {fcRepos} · méthode Karvonen</p>
              </div>
            ) : (
              <div className="rounded-xl border border-dashed border-border/60 bg-muted/10 p-3">
                <p className="text-[11px] text-muted-foreground/70">
                  Renseigne <span className="font-medium">FC Max</span> et <span className="font-medium">FC Repos</span> pour afficher les 5 zones cardio.
                </p>
              </div>
            )}
          </div>
